...
| 2 | ~~**Degenerate metrics propagate to users** — Sharpe `-9.6e16` committed in `docs/data/backtest_report.json:75,676` and `docs/data/strategy_rankings.json:20-25` (division by near-zero std, 1-trade samples); `oos_sharpe` aliased to in-sample at `generate_strategy_data.py:357-358`/`run_full_backtest.py:73-74` so the Darwin promotion gate is constant ×0.1~~<br>RESOLVED 2026-08-18: OOS leak fixed on main — `scripts/generate_strategy_data.py:263-273` now contains a real 80/20 sequential OOS split (`# Defect 2 Fix: compute a REAL out-of-sample signal`, `split_idx = int(len(trades_df) * 0.8)`, train/oos split, `oos_sharpe = safe_sharpe(...)`); `docs/data/backtest_report.json` (2026-08-11) shows sane walk-forward metrics (avg\_is\_sharpe 0.1457, avg\_oos\_sharpe -0.2159, strategy\_sharpe 0.41). Cron `*/5` reference also corrected: `api_health_check.yml` is now daily `0 0 * * *` (see Finding #3). | HIGH (metrics published as fact; README claims have no JSON provenance) | CERTAIN (present in committed data today) |
| 3 | ~~**`api_health_check.yml` burns ~26,000–52,000 min/mo** vs 2,000/mo free tier (`cron */5`, 8,755 runs/mo, full pip install per run)~~<br>RESOLVED 2026-08-18: cron changed to daily `0 0 * * *` (28-31 runs/mo); pip caching added to workflows; heavy backtest jobs on `workflow_dispatch`/reduced frequency; `ci.yml` is pytest-only. | HIGH (CI budget exhaustion kills all workflows) | CERTAIN |
...
|---|---|---|---|---|
| api_health_check.yml | cron `0 0 * * *` (daily) | ~~26,000–52,000~~ **RESOLVED 2026-08-18** | ~~8,755 runs/mo; full pip install + 4 API calls; **13–26× free tier**~~ | ~~hourly/daily cron; pip cache; external uptime monitor~~ |
...
| ~~`docs/data/backtest_report.json:75-76`~~ | ~~`avg_is_sharpe/avg_oos_sharpe` at 1e16 magnitudes~~ **RESOLVED 2026-08-18**: avg\_is\_sharpe 0.1457, avg\_oos\_sharpe -0.2159 (report regenerated 2026-08-11) |
| ~~`docs/data/backtest_report.json:82-88`~~ | ~~`strategy_return_pct: -0.3005` vs `spy_return_pct: 213.1`, `alpha: -213.4`, Sharpe −27.4~~ **RESOLVED 2026-08-18**: strategy\_sharpe 0.41 (report regenerated 2026-08-11) |
| `docs/data/portfolio.json:4-8` | `total_pnl_pct: 99900.0` with `total_trades: 0` |

Producer cause: `comprehensive_backtest_report.py` ~590-597 `get_sharpe` `(excess.mean()/std×sqrt(252)) if std > 0 else 0` — the guard catches only exactly-0 std; near-zero/1-trade std → 1e16 magnitude. ~~**`oos_sharpe` aliased to `train_sharpe`:** `generate_strategy_data.py:357-358`, `run_full_backtest.py:73-74`~~ **RESOLVED 2026-08-18**: real 80/20 sequential OOS split in `generate_strategy_data.py:263-273`; `backtest_report.json` (2026-08-11) shows avg\_oos\_sharpe -0.2159 vs avg\_is\_sharpe 0.1457. README p-values (`0.1100`/`0.2150`, README.md:114-115) have **zero backing artifacts**.
...
- `.github/workflows/api_health_check.yml:6` → `cron: '0 0 * * *'` (daily, RESOLVED 2026-08-18); ~~`'*/5 * * * *'`~~; `:108` → `git commit -am`
- `.github/workflows/self_improvement.yml:49` → `git commit -am`
- `scripts/generate_strategy_data.py:357-358` → ~~`train_sharpe/oos_sharpe = float(sharpe)` ✓ alias~~ **RESOLVED 2026-08-18**: real OOS split at lines 263-273
...
- `docs/data/backtest_report.json` → `report_date 2026-08-11`; ~~is\_sharpe/oos\_sharpe 1e16 magnitudes~~ **RESOLVED 2026-08-18**: avg\_is\_sharpe 0.1457, avg\_oos\_sharpe -0.2159, strategy\_sharpe 0.41