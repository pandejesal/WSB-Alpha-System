...
All brokers implement a `broker_capability` gate (`get_capabilities()`) to verify supported features (e.g., `supports_paper`, `supports_market_orders`) before execution, alongside risk controls: position sizing (`position_sizing.py`), ATR-based stops, circuit breakers, and fails-closed behavior when credentials are missing (no dummy/mock credentials or silent mock fallbacks -- ccxt broker raises `ValueError`/`ImportError` on config miss).
