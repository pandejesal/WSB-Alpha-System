...
All brokers implement risk controls: position sizing (`position_sizing.py`), ATR-based stops, circuit breakers, broker capability gates, and fails-closed behavior when credentials are missing (no dummy/mock credentials or silent mock fallbacks -- ccxt broker raises `ValueError`/`ImportError` on config miss). The broker capability gate is implemented: every broker reports `get_capabilities()` (supports_market_orders, supports_stop_limit, supports_paper, plus auth/balance/order placement/order status/cancel conformance), validated by conformance tests.
