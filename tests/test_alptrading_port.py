...
def test_intraday_buying_power():
    from src.risk.alptrading_safety import (
        IntradayMarginState,
        check_intraday_order,
        intraday_buying_power,
        meet_margin_calls,
        record_margin_call,
    )
    # $100k equity, no exposure: 4x = $400k running BP
    s = IntradayMarginState(start_day_equity=100_000.0)
    assert intraday_buying_power(s) == 400_000.0
    # intraday gains expand BP immediately (new rule; PDT ignored them)
    s.intraday_pnl = 5_000.0
    assert intraday_buying_power(s) == 420_000.0
    # below $2k floor: cash-only 1x
    s2 = IntradayMarginState(start_day_equity=1_500.0)
    assert intraday_buying_power(s2) == 1_500.0
    # pre-trade check rejects deficit-causing orders
    s.gross_exposure = 390_000.0
    v = check_intraday_order(s, 50_000.0)
    assert not v.allowed and "deficit" in v.reasons[0].lower()
    assert check_intraday_order(s, 5_000.0).allowed
    # risk-reducing exits bypass even in deficit
    assert check_intraday_order(s, 50_000.0, risk_reducing=True).allowed
    # de minimis: $100 over on $100k book raises no call
    v = check_intraday_order(IntradayMarginState(), 400_100.0)
    assert v.allowed
    # margin-call escalation: 5 unmet days -> restricted, exits still pass
    st = IntradayMarginState()
    for d in range(5):
        record_margin_call(st, 5_000.0, f"2026-09-0{d + 1}")
    assert st.restricted_until != ""
    assert not check_intraday_order(st, 1_000.0, today="2026-09-05").allowed
    assert check_intraday_order(st, 1_000.0, risk_reducing=True,
                                today="2026-09-05").allowed
    assert meet_margin_calls(st) == 5 and st.restricted_until == ""
