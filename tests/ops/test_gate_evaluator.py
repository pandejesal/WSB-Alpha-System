...
def test_gate_evaluator_g5_passes_weekend_gap():
    ...
    # Safely get exactly 7 trading days across the weekend
    # If bridge is 2 (e.g. Wed, Thu, Fri -> Mon...), bridge - 2 = 0
    # To get 7 days we need indices bridge-2, bridge-1, bridge, bridge+1, bridge+2, bridge+3, bridge+4
    # If bridge is near the start, pad from earlier in seq or shift window right.
    # To be safe and always get 7 items, just take a 7 day slice that contains the bridge:
    start_idx = max(0, bridge - 2)
    week = seq[start_idx:start_idx + 7]
