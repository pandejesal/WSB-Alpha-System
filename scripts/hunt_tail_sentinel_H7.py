#!/usr/bin/env python3
"""H7 tail-sentinel hunt: ALWAYS-INVESTED stress rotation SPY -> duration/metals-4.

Choice (a): harder defensive filter + DD cap on the INTO-leg via universe
restriction. 100% SPY in calm; under stress rotate 100% INTO an equal-weight
basket of the 4 lowest trailing-252d-beta names among the FIXED 5-name
duration/metals sleeve [TLT, GLD, SLV, HYG, UUP] (monthly month-end selection,
past-only, T+1) -- NEVER to cash (tmin=1.00 by construction). Stress =
(SPY close < SMA200) OR (SPY trailing-60d return < -10%), all inputs past-only
shift1. NO VIX input, NO ATR stop, NO realized-vol cap -- outside retired H1
grid; no graduated levels and no 0.00 cash leg -- outside retired H3 ladder;
no broad-panel equity defensive-8 -- outside retired H5 (52% DD).

Replicates evolve_real.backtest() + stat_screens exactly (same lineage as
scripts/hunt_tail_sentinel_H5.py). T+1, tiered W5 costs on BOTH legs, T=1910
bars, per-family N=54 (H5 was 53 -> H7 continues family count). Overlays:
...
RUN_DIR = ROOT / "hunts/tail_risk_sentinel/20260911-h7-tailsentinel"
...
FAM_N = 54  # H1 v4=51, H3=52, H5=53 -> H7 continues the family trial count
...
# Lineage fix vs H5: H5 assigned only the active leg per row then ffill()ed,
# which carried the stale inactive leg forward (stress rows held SPY=1.0 from
# the last calm day PLUS the 1.0 basket = 200% gross after the first stress
# entry). Here each row is written complete: calm -> SPY=1/rest 0; stress ->
# basket 1/K, SPY=0, rest 0; warmup stress w/o basket -> SPY=1. NaN-init kept
# so any unwritten row is detectable (assert below), never silently zero. ----
assets = ["SPY"] + names
...
print(f"h7 sharpe={sharpe:.3f} cagr={cagr(eq):.4f} dd={max_dd(eq):.4f} "
...
eval_data = {
    ...
    "family": FAMILY,
    "params": dict(P, defensive_sleeve=SLEEVE, dd_cap_choice="(a) duration/metals-only INTO-leg"),
...
tmp = RES_DIR / "eval_tail_risk_sentinel_h7.json.tmp"
final = RES_DIR / "eval_tail_risk_sentinel_h7.json"
