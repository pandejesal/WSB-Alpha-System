#!/usr/bin/env python3
"""H11 high-exposure-momentum hunt: inverse-volatility TILT, all-31 always held.

Untried combo (vol63, mom252-21, gain0.5, clip[0.25,2.0], monthly, all-31) on
the 31-name rotation_universe (W13-allowed max breadth; SPY twin ONLY, no
...
perm 0.645 HONEST_ABANDON cycle21 — plus H8 per-name SMA50/200 own-trend
long/half perm 0.695 / excess -14.70pp HONEST_ABANDON cycle23, plus H10
breakout252/confirm63/band0.98/SPY-fill sharpe 0.972 / dd 0.3372 / dsr
0.4537 / perm 0.535 HONEST_ABANDON cycle26. NONE reused here: zero
cross-sectional ranking cutoff (all 31 held every bar), zero SMA (no
SMA50/200, no price-vs-SMA level), zero breakout (no 252d-high proximity,
no confirm gate), no SPY-fill. Continuous tilt: at each monthly signal
date m (past-only closes.loc[:m]) base = 1/vol63 (std of past-63 daily
returns, floor 1e-6); tilt = close[m-21]/close[m-252]-1 per name;
z = cross-sectional z-score; mult = clip(1+0.5*z, 0.25, 2.0);
w = base*mult/sum(base*mult); warmup (<316 bars) holds equal-weight panel.
Gross = 1.0 every post-signal bar so tmin = 1.0 by construction; never
fewer than 20 names (always 31), never cash. Attacks the 5/5 perm wall via
a continuous dual-gradient (vol + momentum) whose time alignment should
degrade under circular shift. Lineage: evolve_real.load_csv /
rotation_universe + _rotation_result (tiered W5 costs, T+1),
check_track_detailed gate, WF 3-fold + CPCV-proxy overlays (frozen params,
returns-level, disclosed). Paper only.
"""
import json
import os
import pathlib
...
RUN_DIR = ROOT / "hunts/high_exposure_momentum/20260911-h11-momentum"
RES_DIR = RUN_DIR / "results"
RES_DIR.mkdir(parents=True, exist_ok=True)

FAMILY = "high_exposure_momentum"
FAM_N = 58  # v1=50..H10=57 -> H11 continues

P = {"vol_lookback": 63, "mom_lookback": 252, "mom_skip": 21,
     "tilt_gain": 0.5, "tilt_clip": [0.25, 2.0],
     "signal_refresh": "monthly", "names_held": 31}


def invvol_momentum_tilt(prices: pd.DataFrame, vol_lb: int, mom_lb: int,
                         skip: int, gain: float, lo: float, hi: float):
    """Monthly inverse-vol + momentum-tilt weights, panel-only, always 1.0.

    Past-only at each monthly signal date m (uses closes.loc[:m] only):
      base_s = 1 / std(past-vol_lb daily returns, floor 1e-6)
      tilt_s = close[m-skip] / close[m-mom_lb] - 1
      z_s = (tilt - mean) / std (0 if std==0)
      mult_s = clip(1 + gain*z_s, lo, hi)
      w[m, s] = base_s * mult_s / sum(base*mult)
      warmup (<316 bars history) -> equal weight 1/N
    NaN-init frames (NOT zeros) + ffill + T+1 shift(1), monthly signal
    ffill daily under W5 tiered costs. No ranking cutoff, no SMA, no
    breakout, no SPY-fill. Gross is 1.0 on every post-signal bar.
    """
    ...
    for m in months:
        ...
        ref_skip = hist.iloc[-(skip + 1)]
        ref_mom = hist.iloc[-(mom_lb + 1)]
        tilt = ref_skip / ref_mom - 1.0
        rets_hist = hist.pct_change().iloc[-vol_lb:]
        vol = rets_hist.std(ddof=1).replace(0.0, np.nan).fillna(1e-6)
        vol = vol.clip(lower=1e-6)
        base = 1.0 / vol
        mu = float(tilt.mean())
        sd = float(tilt.std(ddof=0))
        if sd < 1e-12:
            z = pd.Series(0.0, index=tilt.index)
        else:
            z = (tilt - mu) / sd
        mult = (1.0 + gain * z).clip(lower=lo, upper=hi)
        raw = base * mult
        tot = float(raw.sum())
    ...
    assert n_held_min >= 20, f"tilt must never hold fewer than 20 names: {n_held_min}"
    return port, w, rets
...
port, w, rets = invvol_momentum_tilt(
    prices, P["vol_lookback"], P["mom_lookback"], P["mom_skip"],
    P["tilt_gain"], P["tilt_clip"][0], P["tilt_clip"][1])
res = _rotation_result(port, spy_close, w, rets, FAMILY)
net = res["net"]
print(f"H11 sharpe={res['sharpe']:.3f} dd={res['max_dd']:.4f} oos={res['oos']:.3f} "
...
# Family-continuation DSR (v1=50..H10=57 -> 58), same helper as H2/H4/H6/H8/H10.
try:
...
print(f"dsr_fam(N=58)={dsr_fam:.4f} dsr_gate={dsr_gate:.4f}")
...
eval_data = {
    ...
    "metrics": {"sharpe": res["sharpe"], "cagr": res["cagr"], "max_dd": res["max_dd"],
                ...
                "dsr": dsr_gate, "dsr_fam_N58": round(dsr_fam, 4),
...
tmp = RES_DIR / "eval_high_exposure_momentum_h11.json.tmp"
final = RES_DIR / "eval_high_exposure_momentum_h11.json"
