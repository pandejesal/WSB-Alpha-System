#!/usr/bin/env python3
"""H10 high-exposure-momentum hunt: breakout-persistence + SPY-fill.

Untried combo (breakout252, confirm63, band0.98, monthly, SPY-fill) on the
31-name rotation_universe (W13-allowed max breadth; SPY twin only + SPY-fill
sleeve): TABOO priors are ALL formation-rank rotation — (126,21,5),
...
long/half perm 0.695 / excess -14.70pp HONEST_ABANDON cycle23. NONE reused
here: zero cross-sectional ranking, zero SMA (no SMA50/200, no formation
lookback/skip/top_n). Set-membership breakout-persistence: at each monthly
signal date m (past-only closes.loc[:m]) a name qualifies iff
close[m] >= 0.98 * max252 (max of the 252 closes strictly before m) AND
63d return (close[m]/close[m-63]-1) > 0; warmup (<315 bars) holds SPY-fill.
Slot design: slot i holds name i at 1/31 if qualified else SPY at 1/31, so
gross=1.0 and tmin=1.0 by construction. Attacks BOTH perm-deaths: discrete
rare leadership episodes (not quasi-index weights, not smooth SMA halves)
give the circular-shift perm test sharp timing content. Lineage:
evolve_real.load_csv/rotation_universe + _rotation_result (tiered W5 costs,
T+1), check_track_detailed gate, WF 3-fold + CPCV-proxy overlays (frozen
params, returns-level, disclosed). Paper only.
"""
import json
import os
import pathlib
...
RUN_DIR = ROOT / "hunts/high_exposure_momentum/20260911-h10-momentum"
RES_DIR = RUN_DIR / "results"
RES_DIR.mkdir(parents=True, exist_ok=True)

FAMILY = "high_exposure_momentum"
FAM_N = 57  # v1=50..H6=55, H8=56 -> H10 continues

P = {"breakout_lookback": 252, "confirm_lookback": 63, "proximity_band": 0.98,
     "signal_refresh": "monthly", "fill": "SPY"}


def breakout_persistence_spyfill(prices: pd.DataFrame, spy_close: pd.Series,
                                 lb: int, cl: int, band: float):
    """Monthly breakout-persistence set-membership with SPY-fill slots.

    Past-only at each monthly signal date m (uses closes.loc[:m] only):
      qual = close[m] >= band * max252 AND ret63 > 0  -> 1/N (name slot)
      else                                             -> 0 + 1/N to SPY
      warmup (< lb+cl+1 bars history)                  -> full SPY (1.0)
    NaN-init frames (NOT zeros) + ffill + T+1 shift(1), monthly signal
    ffill daily under W5 tiered costs. No ranking, no SMA, no formation
    window. Gross is 1.0 on every post-signal bar by construction.
    """
    months = first_trading_day_of_month(prices.index)
    n = len(prices.columns)
    cols = list(prices.columns) + ["SPY"]
    ...
    for m in months:
        ...
        k = 0
        for s in prices.columns:
            max252 = hist[s].iloc[-(lb + 1):-1].max()
            prox = bool(last[s] >= band * max252)
            ret63 = float(last[s] / c63[s] - 1.0)
            qual = prox and ret63 > 0.0
            w.loc[m, s] = (1.0 / n) if qual else 0.0
            k += int(qual)
        w.loc[m, "SPY"] = (n - k) / n
        n_set += 1
    ...
    rets_full = rets.copy()
    rets_full["SPY"] = spy_close.pct_change()
    rets_full = rets_full.reindex(w.index).fillna(0.0)
    port = (w * rets_full).sum(axis=1).fillna(0.0)
...
port, w, rets_full = breakout_persistence_spyfill(
    prices, spy_close, P["breakout_lookback"], P["confirm_lookback"],
    P["proximity_band"])
res = _rotation_result(port, spy_close, w, rets_full, FAMILY)
net = res["net"]
print(f"H10 sharpe={res['sharpe']:.3f} dd={res['max_dd']:.4f} oos={res['oos']:.3f} "
...
# Family-continuation DSR (v1=50..H8=56 -> 57), same helper as H2/H4/H6/H8.
try:
...
print(f"dsr_fam(N=57)={dsr_fam:.4f} dsr_gate={dsr_gate:.4f}")
...
eval_data = {
    ...
    "metrics": {"sharpe": res["sharpe"], "cagr": res["cagr"], "max_dd": res["max_dd"],
                ...
                "dsr": dsr_gate, "dsr_fam_N57": round(dsr_fam, 4),
...
tmp = RES_DIR / "eval_high_exposure_momentum_h10.json.tmp"
final = RES_DIR / "eval_high_exposure_momentum_h10.json"
