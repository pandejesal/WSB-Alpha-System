...
def load_base_data():
    ...
    if not os.path.exists(csv_path):
        logger.warning(f"Sentiment data {csv_path} not found. Returning empty signals DataFrame instead of synthesizing data.")
    ...
    unique_tickers = posts_df["ticker"].unique().tolist() if not posts_df.empty else []

    if posts_df.empty:
        # Fallback dates if empty to prevent NaT errors
        min_date = pd.Timestamp.now() - timedelta(days=60)
